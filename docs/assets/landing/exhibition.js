...
for(const shell of document.querySelectorAll('.snap-shell')){const reel=shell.querySelector('.snap-exhibition'),buttons=[...shell.querySelectorAll('[data-snap-index]')];const go=j=>reel.scrollTo({top:clamp(j,0,buttons.length-1)*reel.clientHeight,behavior:reduce.matches?'auto':'smooth'});groupNav(buttons,go);const sync=()=>{const n=Math.round(reel.scrollTop/reel.clientHeight);buttons.forEach((b,i)=>b.setAttribute('aria-pressed',String(i===n)));};reel.addEventListener('scroll',sync,{passive:true});reel.addEventListener('keydown',e=>{if(e.target!==reel)return;let next;if(e.key==='ArrowDown'||e.key==='PageDown')next=Math.round(reel.scrollTop/reel.clientHeight)+1;if(e.key==='ArrowUp'||e.key==='PageUp')next=Math.round(reel.scrollTop/reel.clientHeight)-1;if(next!==undefined){e.preventDefault();go(next);}});}
