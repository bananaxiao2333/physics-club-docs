---
nav_label: "集会原稿"
title: 集会原稿
description: 把时间翻回那一页 —— 2026 年春季的三份集会演示文稿。
nav: ["0421", "0427", "0512"]
icon: lucide/presentation
tags:
  - 2026-04-21
  - 2026-04-27
  - 2026-05-12
# ⚠️ 由 tools/docsgen.py 从 content/collection/gatherings/index.zh-hans.md 生成，请勿手改；要改请改 content/ 下的源文件。
---

# 集会原稿

<p class="memorial-kicker">06 / THE MEMORY COLLECTION · THE ASSEMBLY ROOM</p>

## 把时间，翻回那一页

2026 年春季的三份集会演示文稿。**31 页内容**，保留原有顺序。

!!! info "关于内容口径"
    以下各页均**保留原稿原貌**，包括其中尚未落地的设想。
    方案中的招新、会费与系统设计均视为**历史材料**，本站不提供报名、收款或社团账号服务。

=== "04.21 · 集会暨招新"

    **完整 PDF · 12 页** —— [打开原稿](../../assets/originals/assembly-421.pdf){ target=_blank }

    流程是固定的：发牌 → 纸飞机活动 → 竞赛成果 → 招新 → 自媒体部门 → 会费说明。

    01. 集会封面
    02. 关于社团
    03. 运营人员颁牌
    04. 教师社牌颁发
    05. 会费说明
    06. 科技节纸飞机
    07. 关于招新
    08. 活动照片
    09. 自媒体部门
    10. 竞赛成果展示
    11. Q&A
    12. 感谢聆听

=== "04.27 · 计划的下一页"

    **完整 PDF · 14 页** —— [打开原稿](../../assets/originals/assembly-427.pdf){ target=_blank }

    这一版更新了纸飞机挑战赛时间与报名方式，并第一次完整摊开贡献积分制度的构想。

    01. 纸飞机活动更新
    02. 场地布局
    03. 折法教学展示
    04. 自媒体部门竞选
    05. 积分系统简介
    06. 社费与贡献积分说明
    07. 为什么需要制度
    08. 社费与积分的区别
    09. 系统展示
    10. 积分边界
    11. 制度流程
    12. 实施步骤
    13. 规则展示
    14. 参与和记录

    → 制度正文见 [会费与贡献积分](../../system/governance/fees.md)

=== "05.12 · 向未来的设想"

    **完整 PDF · 5 页** —— [打开原稿](../../assets/originals/assembly-512.pdf){ target=_blank }

    演示文稿出现**奇点、星云、脉冲星、磁陀星**四个小组，以及社团**功勋系统**的展示。

    01. 集会封面
    02. 会员公开白皮书
    03. 功勋系统视频页
    04. 会费记录截图
    05. 小组分组表（原稿隐藏页）

    → 分组名单见 [组织架构与名单](../../system/governance/roster.md)
    → 功勋系统见 [功勋系统](../../system/merit/index.md)

## 逐页原件

### 04.21 集会暨招新

<div class="grid cards" markdown>

- ![集会封面](../../assets/originals/assembly-421-01.jpg)
- ![关于社团](../../assets/originals/assembly-421-02.jpg)
- ![运营人员颁牌](../../assets/originals/assembly-421-03.jpg)
- ![教师社牌颁发](../../assets/originals/assembly-421-04.jpg)
- ![会费说明](../../assets/originals/assembly-421-05.jpg)
- ![科技节纸飞机](../../assets/originals/assembly-421-06.jpg)
- ![关于招新](../../assets/originals/assembly-421-07.jpg)
- ![活动照片](../../assets/originals/assembly-421-08.jpg)
- ![自媒体部门](../../assets/originals/assembly-421-09.jpg)
- ![竞赛成果展示](../../assets/originals/assembly-421-10.jpg)
- ![Q&A](../../assets/originals/assembly-421-11.jpg)
- ![感谢聆听](../../assets/originals/assembly-421-12.jpg)

</div>

### 04.27 计划的下一页

<div class="grid cards" markdown>

- ![纸飞机活动更新](../../assets/originals/assembly-427-01.jpg)
- ![场地布局](../../assets/originals/assembly-427-02.jpg)
- ![折法教学展示](../../assets/originals/assembly-427-03.jpg)
- ![自媒体部门竞选](../../assets/originals/assembly-427-04.jpg)
- ![积分系统简介](../../assets/originals/assembly-427-05.jpg)
- ![社费与贡献积分说明](../../assets/originals/assembly-427-06.jpg)
- ![为什么需要制度](../../assets/originals/assembly-427-07.jpg)

</div>

!!! note "其余各页"
    04.27 与 05.12 的完整逐页图保留在 `docs/assets/originals/`，
    文件名规则为 `assembly-<日期>-<页序>.jpg`，与本页清单一一对应。

## 视频

留存的功勋系统演示视频：`assets/originals/system-demo.mp4`

<video controls preload="metadata" src="../../assets/originals/system-demo.mp4" style="max-width:100%"></video>
